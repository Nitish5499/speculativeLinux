...
// custom system call handler for pnitish
SYSCALL_DEFINE2(pnitish, struct prcs_nitish  __user *, pf, pid_t, pid)
{
	unsigned long ret;
	struct prcs_nitish pif;
	...
	ret = copy_to_user(pf, &pif,  sizeof(struct prcs_nitish));
	if (ret) {
